...
void keyboard_post_init_user(void) {
    ...
    dprintf("\n\n");
    dprintf("========================================\n");
    dprintf("Core Deck Keyboard Initialized (VIAL)\n");
    dprintf("========================================\n");
    dprintf("Firmware: QMK + VIAL\n");
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    /* LT(1, KC_ENT) tap: send Enter via raw HID instead of USB keyboard
     * to bypass macOS IME latency */
    if (keycode == LT(1, KC_ENT) && record->tap.count && record->event.pressed) {
 ...
 * - Layer 3: Transparent (falls through to layer 0)
 */
const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][NUM_DIRECTIONS] = {
    ...
    [3] = { ENCODER_CCW_CW(_______, _______) }                          // Transparent
};
#endif
