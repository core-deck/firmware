    ...
    if (led_min <= 4 && led_max > 4) {
        ...
        switch (mode_state_get()) {
            ...
            case MODE_AUTO:
                ...
                // Default mode has no LED override (reactive glow), so a
                // steady solid gold reads as distinct even though the hue
                // is warm — solid vs. reactive is the real differentiator.
                mode_hsv.h = 32;
                break;
            default:
